...
This is a brand-new `uv`-managed Python project (src layout), published on PyPI as
`narrately`. The only code so far is a `main()` stub in `src/narrately/__init__.py` that
prints a greeting. There are no tests, no `tests/` directory, and no CI configured yet —
set these up as the project grows rather than assuming they exist.
...
```bash
uv run narrately             # run the CLI entry point (defined in [project.scripts])
...
Only `src/narrately/__init__.py` exists today, so this structure hasn't been built out yet
